...
__author__ = 'Jennie Lees, rewritten: TheMisterSenpai'
__version__ = '1.4b'
...
class LeagueOfLegends:
    ...
    def __webrequest(self, url):
        ...
        try:
            opener = urllib3.build_opener(NotModifiedHandler())
            ...
            if url in self.__cache:
                ...
                if 'etag' in self.__cache[url]:
                    print ('Adding ETag to request header: ' + self.__cache[url]['etag'] + req.add_header('If-None-Match', self.__cache[url]['etag']))
                if 'last_modified' in self.__cache[url]:
                    print ('Adding Last-Modified to request header: ' + self.__cache[url]['last_modified'] + req.add_header('If-Modified-Since', self.__cache[url]['last_modified']))
...
class RiotError(Exception):
    def __init__(self, code):
        self.code = code
        if code == 429:
            self.error_msg = print("Rate limit exceeded. Riot smash.")
        else:
            self.error_msg = print("Error %s from Riot servers." % code)
...
'''
class NotModifiedHandler(urllib3.BaseHandler):

    def http_error_304(self, req, fp, code, message, headers):
        addinfourl = urllib3.addinfourl(fp, headers, req.get_full_url())
